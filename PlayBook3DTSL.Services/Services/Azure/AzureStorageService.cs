...
namespace Playbook.Service.Service.AzureCommand
{
        ...
        public async Task UploadeFileAsync(List<CloudServiceModel> cloudServiceModel)
        {
            foreach (var file in cloudServiceModel)
            {
                if (file.IsFromSourcePath)
                {
                    ...
                    BlobServiceClient blobServiceClient = new BlobServiceClient(_appconfiguration.BlobStorageConnection);
                    BlobContainerClient containerClient = blobServiceClient.GetBlobContainerClient(_appconfiguration.BlobStorageCotainer);
                    BlobClient blobClient = containerClient.GetBlobClient(file.CoudPath + "/" + file.FileName);
                ...
                {
                    BlobClient blob = _container.GetBlobClient(file.CoudPath + "/" + file.FileName);
